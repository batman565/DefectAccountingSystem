...
func main() {
	...
	fileService := services.NewFileService(fileRepo, "../backend/storage")
	...
	mux.Handle("/storage/", http.StripPrefix("/storage/", http.FileServer(http.Dir("../backend/storage"))))
	...
	mux.HandleFunc("/api/users/update", jwt.RequireSupervisorOrManager(personHandler.UpdatePersonHandler))
	...
	mux.HandleFunc("/api/users/get/all", jwt.RequireManagerOrEngineerOrSupervisor(personHandler.AllPersonHandler))
	mux.HandleFunc("/api/objects/create", jwt.RequireSupervisorOrManager(objectHandler.CreateObjectHandler))
	...
	mux.HandleFunc("/api/defects/get/all", jwt.RequireSupervisorOrManager(defectHandler.GetAllDefectsHandler))
	...
	mux.HandleFunc("/api/defects/get/regperson", jwt.RequireEngineer(defectHandler.GetDefectsByRegPersonID))
	mux.HandleFunc("/api/defects/get/doperson", jwt.RequireEngineer(defectHandler.GetDefectsByDoPersonID))
	mux.HandleFunc("/api/comments/create", jwt.RequireManagerOrEngineer(commentHandler.CreateCommentHandler))
	mux.HandleFunc("/api/comments/get/defect", jwt.RequireManagerOrEngineerOrSupervisor(commentHandler.GetCommentsHandler))
	mux.HandleFunc("/api/files/create", jwt.RequireManagerOrEngineer(fileHadler.UploadFile))
	mux.HandleFunc("/api/files/get", jwt.RequireManagerOrEngineerOrSupervisor(fileHadler.GetFilesByDefect))
	mux.HandleFunc("/api/hed/get", jwt.RequireManagerOrEngineerOrSupervisor(hedHandler.GetHEDByID))
